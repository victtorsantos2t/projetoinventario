...
export default function DashboardPage() {
  ...
  const getDashboardData = async () => {
    try {
      ...
      let query = supabase.from('ativos').select('status')

      if (isViewer) {
        if (profile?.setor_id) {
          query = query.eq('setor_id', profile.setor_id)
        } else if (profile?.setor) {
          const sectorName = typeof profile.setor === 'string' ? profile.setor : (profile.setor as any).nome
          if (sectorName) query = query.eq('setor', sectorName)
        }
      }
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="h-32 rounded-2xl bg-white border border-slate-100 shadow-sm animate-pulse" />
          ))}
        </div>
      ) : (
